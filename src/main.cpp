        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); ++i) {
            ...
            if (car_lane == lane) {
              ...
              // object car is left from our car
              car_left = car_left | ((car_s-30 < check_car_s) && (car_s+30 > check_car_s));
            }
            else if (car_lane - lane == 1) {
              // object car is right from our car
          ...
          if (car_ahead) {
            ...
            if (!car_left && lane>0) {
              // check if there is no car at left lane and we have a left lane to move to
              lane--; // change to left lane
            }
            else if (!car_right && lane !=2) {
              // check if there is no car at right lane and we are not at the fartest right lane
              lane++; // change to right lane
            }
            else { // if we have other cars at both right and left lane which means no lane to change, just slow down
              speed_diff -= max_acc;
            }
          }
          else {
          ...
          // later we will interpolate these waypoints with a spline function and fill it in with more points that control speed
          vector<double> ptsx;
          vector<double> ptsy;
